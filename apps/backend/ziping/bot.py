...
import sys
sys.path.append(r'C:\Users\김승원\Desktop\KR_SSA\common')
...
class MyBot(ActivityHandler):
    ...
    async def on_message_activity(self, turn_context: TurnContext):
        ...
        # Set LLM 
        llm = AzureChatOpenAI(deployment_name=self.model_name, temperature=0.1, max_tokens=4000, callback_manager=cb_manager)

        # Initialize our Tools/Experts
        text_indexes = ["cogsrch-index-files", "cogsrch-index-csv"]
        doc_search = DocSearchTool(llm=llm, indexes=text_indexes,
                           k=3, similarity_k=2, reranker_th=1,
